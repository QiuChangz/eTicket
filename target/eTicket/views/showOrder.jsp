    ...
    <table width="100%" class="table table-hover">
        ...
        <tr>
            ...
            <td width="100">
                ...
                %>
                <mvc:form method="get"  name="enter" action = "<%=\"/eTicket/editEvaluation/\"+order.getId()%>">
                    <input type="submit" class="btn btn-default" value="修改评论" >
                </mvc:form>
                <%
                    }else if (order.getState().equals("支付中")){
                %>
                <mvc:form method="post"  name="enter" action = "payOrder">
                    <input type="hidden" name="orderId" value="<%=order.getId()%>">
                    <input type="submit" class="btn btn-default" value="支付" >
                </mvc:form>

                <mvc:form method="post"  name="enter" action = "cancel">
                    <input type="hidden" name="orderId" value="<%=order.getId()%>">
                    <input type="submit" class="btn btn-default" value="取消订单" >
                </mvc:form>
                <%
                    }else{
                %>
